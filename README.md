...
Плагин запускает сегментацию SAM3 прямо в QGIS.
Для начала нужно установить модель SAM3 
(`[facebook/sam3](https://huggingface.co/facebook/sam3/tree/main (нужно зарегистрироваться и клонировать репозиторий (скачать файлы локально на компьютер)
или использовать зеркало: https://huggingface.co/buckets/haibozzz/sam3-bucket)`, https://disk.360.yandex.ru/d/0fhHpPyOPWrg0g или взять отсюда).
Поддерживается текстовый промпт или клик/бокс на карте (лучше работать с текстом) → векторный слой с полигонами в текущем
